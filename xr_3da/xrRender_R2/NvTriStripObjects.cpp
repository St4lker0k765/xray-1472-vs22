...
bool NvStripInfo::Unique(NvFaceInfoVec& faceVec, NvFaceInfo* face)
{
	...
	for(int i = 0; i < faceVec.size(); i++)
	{
		...
		//the face is not unique, all it's vertices exist in the face xr_vector
		if(bv0 && bv1 && bv2)
			return false;
	}
...
void NvStripInfo::Build(NvEdgeInfoVec &edgeInfos, NvFaceInfoVec &faceInfos){
	...
	// Combine the forward and backwards stripification lists and put into our own face xr_vector
...
// RemoveSmallStrips()
//
// allStrips is the whole strip xr_vector...all small strips will be deleted from this list, to avoid leaking mem
...
//
// Splits the input xr_vector of strips (allBigStrips) into smaller, cache friendly pieces, then
...
// AvgStripSize()
//
// Finds the average strip size of the input xr_vector of strips
//
...
// FindAllStrips()
//
// Does the stripification, puts output strips into xr_vector allStrips
//
